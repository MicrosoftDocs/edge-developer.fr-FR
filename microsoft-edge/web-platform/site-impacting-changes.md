...
Examinez souvent cet article.  L Microsoft Edge de travail met à jour cet article à mesure que la réflexion évolue, que les chronologies s’unifient et que de nouvelles modifications sont annoncées.  

| Modification | Canal Stable | Expérimentation | Informations complémentaires |  
|:--- |:--- |:--- |:--- |
| Deprecate WebRTC’s Plan B SDP semantics | [Chrome+1](#release-comments) \(Edge v94\)  |  | Cette modification se produit dans le projet Chromium, sur lequel Microsoft Edge est basé. Cette modification est déprécié dans un dialecte SDP (Session Description Protocol) hérité appelé Plan B. Il est remplacé par le plan unifié, qui est un format SDP compatible avec les spécifications et entre navigateurs. Pour plus d’informations, accédez à Entrée d’état de la plateforme [Chrome][ChromestatusFeature5823036655665152] et PSA : Chronologie de l’annulation et de la suppression de SDP plan B - Veuillez migrer vers [le plan unifié.][PSADeprecateWebRTCPlanB] La planification du déploiement de Microsoft pour l’annulation est prévue pour une version après Chrome. La demande d’un jeton d’essai [webRTC Plan B Reverse Origin][ChromeDevelopersOrigintrialsWebRTCPlanBOriginTrial] permet aux sites de continuer à utiliser l’API dépréciée jusqu’à Edge v96. |
