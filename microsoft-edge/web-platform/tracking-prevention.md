---
description: Cette page fournit de la documentation sur la fonctionnalité de prévention Microsoft Edge (Chromium)
...
La fonctionnalité de prévention du suivi dans Microsoft Edge protège les utilisateurs du suivi en ligne en limitant la capacité des suivis à accéder au stockage basé sur le navigateur ainsi qu’au réseau.  Il est conçu pour respecter [][MicrosoftEdgeBrowserPrivacyPromise] la promesse de confidentialité du navigateur Microsoft Edge tout en garantissant l’absence d’impact par défaut sur la compatibilité des sites web ou la politique économique du web.  
...
1.  **Application** : mesures prises pour protéger les utilisateurs Microsoft Edge des URL classées comme suivis.  
...
Si l’un de ces noms d’hôte correspond à un nom d’hôte dans les listes De déconnexion, Microsoft Edge passe à l’évaluation des actions d’application afin d’empêcher le suivi des utilisateurs. [][|::ref2::|Main] [][GitHubDisconnectMeTrackingProtection]  
...
La façon dont les mesures d’application sont appliquées dépend du niveau de prévention du suivi sélectionné par l’utilisateur et des atténuations qui peuvent s’appliquer.  
...
En fonction des commentaires des utilisateurs récents reçus par Microsoft, Microsoft Edge conserve une petite liste de sites \(dont la plupart font partie de la catégorie Déconnecter le contenu\) qui ont été rompus en raison de la prévention du suivi, bien que les deux préventions ci-dessus soient en place. Les sites de cette liste ne sont pas exempts des mesures de prévention du suivi.  La liste se trouve sur le disque aux [emplacements décrits](#determining-whetherhow-a-particular-url-is-classified) ci-dessous.  Les utilisateurs peuvent remplacer les entrées qui y sont entrées à l’aide de **l’option** Bloquer dans `edge://settings/content/cookies` .

Pour éviter de conserver cette liste, Microsoft travaille actuellement sur [l’API Stockage Access][GitHubMsExplainersStorageAccessApi] dans la base de code Chromium base de données.  [L’API Stockage Access][GitHubMsExplainersStorageAccessApi] permet aux développeurs de sites de demander directement aux utilisateurs l’accès au stockage, en fournissant aux utilisateurs une plus grande transparence sur la façon dont leurs paramètres de confidentialité affectent leur expérience de navigation et en offrant aux développeurs de sites des contrôles pour se débloquer rapidement et de manière intuitive.

Une fois [l’API Stockage Access][GitHubMsExplainersStorageAccessApi] implémentée, Microsoft désintéresse la liste CompatExceptions et atteint les sites affectés pour les rendre informés des problèmes et leur demander d’utiliser l’API Stockage [Access][GitHubMsExplainersStorageAccessApi] à l’avenir.  
...
| | Désélation | Analytique | Contenu | Cryptomining | Empreinte digitale | Social | Other | Prévention de la même organisation | Atténuation de l’engagement de l’organisation |  
| - | - | - | - | - | - | - | - | - | - | - |  
| **De base** | - | - | - | B | B | - | - | Activé | N/A |  
| **Équilibrée** | S | - | S | B | B | S | S | Activée | Activée |  
...
Dans Microsoft Edge 79, le comportement par défaut était d’appliquer des protections en mode strict dans InPrivate.  Dans Microsoft Edge 80, ce comportement a été remplacé par un commutateur qui permet aux utilisateurs de décider s’il faut appliquer des protections en mode strict ou conserver leurs paramètres réguliers lors de la navigation `edge://settings/privacy` dans InPrivate.  
...
>
> **Répertoire d’installation** : listes regroupées avec le programme Microsoft Edge installer.  Si vous avez sélectionné un répertoire d’installation différent, vos chemins d’accès exacts peuvent être différents.  
>
...
La section suivante contient les réponses aux questions fréquemment posées sur la fonctionnalité de prévention du suivi dans Microsoft Edge.  
...
[ChromiumDesignDocsSiteEngagement]: https://www.chromium.org/developers/design-documents/site-engagement "Engagement de site : Chromium projets"  
...
[GitHubMsExplainersStorageAccessApi]: https://github.com/MicrosoftEdge/MSEdgeExplainers/blob/master/StorageAccessAPI/explainer.md "Stockage Access API Explainer - MSEdgeExplainers/StorageAccessAPI | GitHub"
