...
*   Événement étendu qu’il est maintenant élevé pour les processus enfants `ProcessFailed` non-renderer et les rendueurs d’images.  
*   Ajout du [paramètre expérimental AreBrowserAcceleratorKeysEnabled.][Webview2ReferenceWin32Icorewebview2experimentalsettingsViewWebview210824GetArebrowseracceleratorkeysenabled]  Vous pouvez choisir d’empêcher le navigateur de répondre aux touches d’accès rapide liées à la navigation, l’impression, l’enregistrement et d’autres fonctions spécifiques au navigateur.  
*   Ajout de la prise en charge de l’iframe pour `AddScriptToExecuteOnDocumentCreated` .  
...
*   Correction Visual Studio’exécution lors de `WebView2.Dispose` l’exécution lorsqu’il est hébergé dans plusieurs fenêtres.  \([\#816][GithubMicrosoftedgeWebviewfeedbackIssue816]\ et [\#442][GithubMicrosoftedgeWebviewfeedbackIssue442]\).  
...
*   Correction d’un bogue qui s’est produit sur les applications WebView qui utilisent le SDK WPF.  L’incident s’est produit lorsque des fenêtres ont été fermées à l’aide de `F4` la clé.  \([\#399][GithubMicrosoftedgeWebviewfeedbackIssue399]\).  
