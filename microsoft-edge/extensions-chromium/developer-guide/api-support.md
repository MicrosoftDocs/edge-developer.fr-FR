...
---
# <a name="supported-apis-for-microsoft-edge-extensions"></a>API pris en charge pour Microsoft Edge extensions
...
| [browserAction](https://developer.chrome.com/extensions/browserAction) | Utilisez les actions du navigateur pour placer des icônes dans la barre d’outils Microsoft Edge. Vous pouvez également utiliser des actions de navigateur pour ajouter une boîte à outils, un badge ou une fenêtre popup. |
...
| [contentSettings](https://developer.chrome.com/extensions/contentSettings) | En règle générale, les paramètres de contenu vous permettent de personnaliser le comportement des Microsoft Edge sur chaque site, et non globalement. Modifier les paramètres qui contrôlent si les sites web peuvent utiliser des fonctionnalités telles que les cookies, JavaScript et les plug-ins. |
| [contextMenus](https://developer.chrome.com/extensions/contextMenus) | Ajoutez des éléments au menu context Microsoft Edge. Les éléments de menu peuvent s’appliquer à différents objets, tels que des images, des liens hypertexte et des pages. |
...
| [declarativeContent](https://developer.chrome.com/extensions/declarativeContent) | Prendre des mesures en fonction du contenu d’une page, sans avoir besoin d’autorisation pour lire le contenu de la page. |
| [declarativeNetRequest](https://developer.chrome.com/extensions/declarativeNetRequest) | Fournit davantage de confidentialité en bloquant ou en modifiant les demandes réseau en spécifiant des règles déclaratives. Autoriser les extensions à modifier les demandes réseau sans intercepter la demande et afficher le contenu. |
...
| [enterprise.hardwarePlatform](https://developer.chrome.com/extensions/enterprise.hardwarePlatform) | Obtenez le fabricant et le modèle de la plateforme matérielle sur laquelle le navigateur s’exécute. Cette API est uniquement disponible pour les extensions installées par la stratégie d’entreprise. |
...
| [extensionTypes](https://developer.chrome.com/extensions/extensionTypes) | Contient des déclarations de type pour Microsoft Edge extensions. |
...
| [notifications](https://developer.chrome.com/extensions/notifications) | Créez des notifications enrichies à l’aide de modèles et affichez-les dans le bac système. |
| [omnibox](https://developer.chrome.com/extensions/omnibox) | Inscrivez des mots clés dans la Microsoft Edge d’adresses, également appelée omnibox. |
| [pageAction](https://developer.chrome.com/extensions/pageAction) | Ajoutez des icônes à Microsoft Edge barre d’outils, à droite de la barre d’adresses. Les actions de page sont des actions qui peuvent être entreprises sur la page actuelle et qui ne sont pas applicables à toutes les pages. Les actions de page apparaissent grisées lorsqu’elles sont inactives. |
...
| [protection des données personnelles](https://developer.chrome.com/extensions/privacy) | Contrôlez les fonctionnalités Microsoft Edge qui affectent la confidentialité d’un utilisateur. Cette API dépend du `EdgeSetting` prototype de `types` l’utilisation et de la configuration de Microsoft Edge. |
| [proxy](https://developer.chrome.com/extensions/proxy) | Gérer les paramètres de proxy pour Microsoft Edge. Cette API dépend du prototype de l’API pour obtenir et définir la configuration proxy de `EdgeSetting` `types` Microsoft Edge. |
...
## <a name="unsupported-extension-apis"></a>API d’extension non pris en place
...
* L’utilisateur doit être Microsoft Edge à l’aide d’un compte MSA Azure Active Directory à `chrome.identity.getProfileUserInfo` utiliser. Si l’utilisateur est Microsoft Edge à l’aide d’un compte Active Directory local, l’API renvoie les valeurs de courrier électronique et `null` d’ID.

* Microsoft Edge ne prend pas en charge les extensions qui utilisent des paiements Chrome Web Store, car il utilise pour demander des jetons pour les `identity.getAuthtoken` utilisateurs qui se sont inscrits. Ces jetons sont envoyés à l’API de licence basée sur REST. 
