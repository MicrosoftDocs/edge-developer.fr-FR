...
Après avoir [réservé le nom de votre extension](./extensions-in-the-windows-dev-center.md#name-reservation) par le biais du centre de développement Windows, vous pouvez trouver les informations d’identité du package nécessaires pour remplacer les valeurs suivantes dans AppXManifest. xml:
...
Certaines valeurs dans le AppXManifest doivent correspondre à celles définies dans le manifeste JSON. Vous pouvez mettre à jour les valeurs suivantes dans appxmanifest. XML en fonction du manifeste de votre extension JSON:
...
Une fois que vous avez créé les ressources d’interface utilisateur nécessaires, mettez à jour AppXManifest. XML de façon à ce qu’ils pointent vers les fichiers appropriés:
