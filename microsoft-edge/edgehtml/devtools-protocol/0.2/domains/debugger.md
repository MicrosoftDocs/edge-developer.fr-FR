...
---
# <a name="debugger-domain---devtools-protocol-version-02-edgehtml"></a>Domaine du débogueur - DevTools Protocol Version 0.2 (EdgeHTML)  
...
| Parameters | Type | Détails |  
|:--- |:--- |:--- |  
| state | `string` | Mettre en pause le mode exceptions.  Valeurs autorisées  `none` : , `uncaught` , `all` |  
...
| Parameters | Type | Détails |  
|:--- |:--- |:--- |  
| debuggerPropertyId | chaîne | Microsoft : ID de propriété \(c’est-à-dire `msDebuggerPropertyId` \) à définir. | 
| newValue | `string` | &nbsp; |  
...
Déclenché lorsque les débompeurs se cassent pour un point d’arrêt ou une exception.  

| Parameters | Type | Détails |  
|:--- |:--- |:--- |  
| callFrames | [CallFrame[]](#callframe) | Pile d’appels sur le débogger arrêté. |  
| reason | `string` |  Motif de l’interruption.  Valeurs autorisées  `breakpoint` : `step` , , `exception` `other` et `EventListener` |  
...
| asyncStackTrace \(optional\) | `StackTrace` | Suivi de pile async JavaScript. |  
...
| columnNumber \(optional\) | `integer` | Numéro de colonne dans le script \(0-based\). |  
| msLength | `integer` | Microsoft : Longueur du code \(c’est-à-dire le nombre de caractères\) à ce cadre d’appel. |  
| type \(facultatif\) | `string` | Valeurs autorisées : debuggerStatement, call, return. |  
...
Image d’appel JavaScript. Le tableau des trames d’appel forme la pile d’appels.  
...
| Propriétés | Type | Détails |  
|:--- |:--- |:--- |  
| type | `string` | Type d’étendue.  Valeurs autorisées  `global` : , , , , , , `local` , `with` `closure` `catch` `block` `script` `eval` `module` et `return` |  
