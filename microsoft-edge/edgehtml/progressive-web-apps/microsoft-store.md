...
3.  Quand vous chargez les packages de votre application, les valeurs *DisplayName*, *Name*, *Publisher*et *PublisherDisplayName* dans votre fichier *. appxmanifest* doivent correspondre aux valeurs affectées à votre application dans le tableau de bord du centre de développement Windows, au moment de réserver le nom. 
    ...
    Connectez-vous au [tableau de bord du centre de développement Windows](https://developer.microsoft.com/dashboard/windows/overview)et recherchez les valeurs d’identité de votre application sous identité de l’application gestion des **applications**  >  ****:
...
-   Un fichier manifeste de l’application Web non vide avec au minimum:
    
    -   Nom
    -   Description
    -   Icône d’application d’au moins 512 x 512 pixels
        
-   Logique principale unique (pas de code à modification minimum d’un modèle [réutilisable](https://en.wikipedia.org/wiki/Boilerplate_code) par une application)
-   Être fourni sur une connexion sécurisée (HTTPs)
-   Script (s) de travailleur de service
-   Ne violer aucune législation ou politique de la [Boutique Microsoft](/legal/windows/agreements/store-policies).
