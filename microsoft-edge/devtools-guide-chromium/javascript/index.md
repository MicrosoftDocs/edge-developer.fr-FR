...
1.  De retour à la démonstration, choisissez de nouveau **Ajouter le numéro 1 et le numéro 2.**  DevTools interrompt la démonstration et met en évidence une ligne de code dans **l’outil Sources.**  DevTools doit s’interrompre sur la ligne 16 dans `get-started.js` .  
    ...
    Si vous faites une pause sur une autre ligne de code, choisissez Reprendre l’exécution du **script** \( Reprendre l’exécution du script \) jusqu’à ce que vous arrêtiez ![ sur la ligne ](../media/resume-script-run-icon.msft.png) correcte.  
...
> [!NOTE]
> Dans le **volet Expression** de l’observation \(en bas à droite\) de la figure suivante, l’Expression `typeof sum` d’observation s’affiche.  Si votre fenêtre DevTools est grande, le volet **Expression** de l’observation se trouve à droite au-dessus du volet Points d’arrêt de l’écoute d’événements. ****  
