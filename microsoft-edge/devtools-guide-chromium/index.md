...
---
# Vue d’ensemble des outils de développement Microsoft Edge (chrome)  
...
Si vous consultez la nouvelle version de Microsoft Edge et que vous avez déjà développé dans Microsoft Edge \ (EdgeHTML \), vous disposez maintenant de nouveaux outils très pratiques qui vous permettent de créer et tester plus facilement vos sites Web dans Microsoft Edge.  

## Ouvrir le DevTools  

Si vous n’avez jamais utilisé le DevTools auparavant, les outils de développement Microsoft Edge sont un ensemble d’outils intégrés directement dans le navigateur Microsoft Edge.  Avec DevTools, vous pouvez effectuer les opérations suivantes.  
...
*   Sélectionner `F12` 
*   Sélectionnez `Ctrl` + `Shift` + `I` Windows/Linux \ ( `Command` + `Option` + `I` sur MacOS \)  
...
Si vous voulez afficher les journaux de votre code JavaScript frontal ou exécuter rapidement un script, sélectionnez `Ctrl` + `Shift` + `J` sur Windows/Linux ou `Command` + `Option` + `J` sur MacOS pour lancer le panneau **console** dans le devtools.  
...
Dans la partie supérieure, sélectionnez **autoriser les extensions d’autres magasins** , puis choisissez **autoriser** dans la boîte de dialogue qui s’affiche.  
...
| Afficher le DevTools dans le **mode inspecter l’élément** , qui vous permet de choisir un élément sur le site et d’afficher le code HTML et CSS dans le panneau **éléments** | `Ctrl`+`Shift`+`C` | `Command`+`Option`+`C` |  
...
| Ancrez le DevTools à la dernière position utilisée.  Si la DevTools reste dans la position par défaut de la session entière, le raccourci détache la DevTools dans une fenêtre séparée | `Ctrl`+`Shift`+`D` | `Command`+`Shift`+`D` |  
...
| Mise.  Fefreshes la page à l’aide du cache.  | `F5` ou `Ctrl`+`R` | `Command`+`R` |  
| Actualisation matérielle.  Force Microsoft Edge à télécharger de nouveau des ressources et à le recharger.  Il est possible que les ressources utilisées peuvent provenir d’une version mise en cache | `Ctrl`+`F5` ou `Ctrl`+`Shift`+`R` | `Command`+`Shift`+`R` |  
