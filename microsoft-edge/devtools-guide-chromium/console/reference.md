...
Pour ouvrir **l’outil Console** à partir du [menu][DevtoolsCommandMenuIndex]Commande, tapez, puis exécutez la commande Afficher la console qui a le `Console` badge **Panneau** en côté. ****  
...
Pour ouvrir **l’outil Console** à partir du [menu][DevtoolsCommandMenuIndex]Commande, tapez, puis exécutez la commande Afficher la console qui a le badge à l’aide `Console` de celui-ci. **** ****  
...
*   [Autocomplete de l’historique](#turn-off-autocomplete-from-history)  
...
L’action par défaut Microsoft Edge consiste à enregistrer les messages réseau dans la **console.**  Dans la figure suivante, le message choisi représente un code d’état HTTP de `429` .  
...
1.  [Ouvrez console Paramètres](#open-console-settings).  
...
1.  Développez la section **# messages de l’utilisateur.**  
...
L’option par défaut pour la liste de listes des contextes **JavaScript** est **supérieure,** ce qui représente le contexte de navigation [de][MdnDocsGlossaryBrowsingContext] la page web principale.  
