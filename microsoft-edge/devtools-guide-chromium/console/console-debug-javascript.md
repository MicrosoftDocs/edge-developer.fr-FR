...
Choisissez le bouton qui vous permet d'aller sur la **console** et de vous donner plus d'informations sur l'erreur.  
...
*   `Uncaught (in promise) TypeError: data.forEach is not a function`Erreur.  
...
:::image type="complex" source="../media/console-debug-network-error-fixed.msft.png" alt-text="L'exemple sans erreur charge les informations à partir de GitHub et les affiche" lightbox="../media/console-debug-network-error-fixed.msft.png":::
   L'exemple sans erreur charge les informations à partir de GitHub et les affiche  
:::image-end:::  

Veillez à fournir des techniques de codage peu techniques pour éviter les expériences utilisateur précédentes.  Assurez-vous également que votre code capture les erreurs et affiche chacune d'elles dans la **console.**  Accédez au [rapport d'erreurs réseau dans la console et l'interface][GithubMicrosoftedgeDevtoolssamplesConsoleNetworkErrorReportedHtml] utilisateur et examinez les éléments suivants.  

*   Fournissez l'interface utilisateur à l'utilisateur pour qu'un problème se soit passé.  
...
Vous avez trois fonctions qui se demandent l'un l'autre successivement.  
...
Si vous ne savez pas quelle fonction demande une autre fonction, utilisez la méthode pour suivre les fonctions qui sont demandées pour `console.trace()` obtenir la fonction actuelle.  Pour afficher le suivi dans la **console,** accédez à [Création de suivis dans la console.][GithubMicrosoftedgeDevtoolssamplesConsoleTraceHtml]  
...
Le résultat est une trace à afficher qui est nommée, puis dans le `here()` `there()` deuxième exemple `everywhere()` qu'il est nommé `everywhere()` .  
