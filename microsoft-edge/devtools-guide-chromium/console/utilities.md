...
> [!WARNING]
> Les commandes suivantes fonctionnent uniquement dans la **** console DevTools de Microsoft Edge.  Les commandes ne fonctionnent pas si elles sont exécutés à partir de vos scripts.  

Pour plus d'informations sur les méthodes et le reste des méthodes, accédez à Référence de `console.log()` `console.error()` `console.*` [l'API console.][DevToolsConsoleApi]  
...
>[!NOTE]
> Le [problème Chromium #1050237][CR1050237] suivi d'un bogue avec la `debug()` fonction.  Si vous rencontrez le problème, essayez plutôt d'utiliser des [points d'arrêt.][DevtoolsJavascriptBreakpoints]  
...
Pour plus d'informations sur les points d'arrêt, accédez à Comment suspendre votre code avec des points d'arrêt dans [Microsoft Edge DevTools][DevtoolsJavascriptBreakpoints].  
...
:::image type="complex" source="../media/console-monitor-events-resize-window.msft.png" alt-text="Événements de resize de fenêtre de surveillance" lightbox="../media/console-monitor-events-resize-window.msft.png":::
   Événements de resize de fenêtre de surveillance  
:::image-end:::  

L'extrait de code suivant définit un tableau pour surveiller à la fois et les événements `resize` `scroll` sur l'objet fenêtre.  
