...
1.  Sélectionnez **Afficher** et choisissez **dans une nouvelle fenêtre**.  La démonstration s’ouvre dans un nouvel onglet.  Cet onglet est appelé **onglet démonstration**.  Le chargement du site risque de prendre un certain temps.  
...
1.  Sélectionnez `Control` + `Shift` + `J` \ (Windows, Linux \) ou `Command` + `Option` + `J` \ (MacOS \).  Microsoft Edge DevTools s’ouvre à côté de la démonstration.  
...
Pour le reste des captures d’écran de ce didacticiel, DevTools est affiché dans une fenêtre séparée.  Sélectionnez `Control` + `Shift` + `P` \ (Windows, Linux \) ou `Command` + `Shift` + `P` \ (MacOS \) pour ouvrir le menu de commandes, `Undock` puis sélectionnez **détacher dans une fenêtre séparée**.  
...
Sélectionnez le bouton bascule surligné dans la figure ci-dessous pour afficher une description de chaque métrique, puis sélectionnez **en savoir plus** pour lire la documentation correspondante.  
...
Pour en **savoir plus** , voir documentation relative aux raisons pour lesquelles l’opportunité est importante et recommandations en matière de correction.  
...
1.  Si vous ne voyez pas la colonne **taille** dans le tableau des requêtes réseau, cliquez sur l’en-tête de tableau, puis sélectionnez **taille**.  
...
1.  Dans l’onglet Éditeur, sélectionnez **server.js**.  
...
1.  Dans votre rapport, sélectionnez **éviter les charges utiles réseau énormes** pour afficher les images qui doivent être redimensionnées.  Il semble que 2 des fichiers jpg dépassent 2000 Ko, ce qui est plus grand que nécessaire.  
...
1.  Sélectionnez `Control` + `Shift` + `P` \ (Windows, Linux \) ou `Command` + `Shift` + `P` \ (MacOS \) pour ouvrir le menu de commandes, commencez à taper `Coverage` , puis sélectionnez **afficher la couverture**.  
...
1.  Cliquez sur **Actualiser** , puis sur ![ Actualiser ][ImageRefreshIcon] .  L’onglet **couverture** fournit une vue d’ensemble de la quantité de code `bundle.js` `jquery.js` et `lodash.js` s’exécute pendant le chargement de la page.  Dans l’illustration ci-dessous, à propos de 76% et de 30% des fichiers jQuery et Lodash ne sont pas utilisés, respectivement.  
...
1.  Sélectionnez l’onglet **Network (réseau** ).  
1.  Sélectionnez `Control` + `Shift` + `P` \ (Windows, Linux \) ou `Command` + `Shift` + `P` \ (MacOS \) pour ouvrir de nouveau le menu de commandes.  
...
1.  Choisissez **Ajouter un modèle** \ ( ![ Ajouter le modèle ][ImageAddPatternIcon] \), tapez `/libs/*` , puis sélectionnez `Enter` pour confirmer.  
...
1.  Pour supprimer le modèle de blocage, choisissez **Supprimer tous les modèles** \ ( ![ Supprimer tous les modèles ][ImageRemoveIcon] \) `/libs/*` .  
...
1.  Choisissez **paramètres de capture** \ ( ![ paramètres de capture ][ImageCaptureIcon] \).  
...
1.  Cliquez sur **Actualiser** , puis sur ![ Actualiser ][ImageRefreshIcon] .  DevTools actualise la page, puis produit une visualisation de tout le temps exécuté pour le chargement de la page.  Cette visualisation est appelée **suivi**.  
...
1.  Choisissez de nouveau **minutage** pour réduire cette section.  
